...
impl TryFrom<notionrs_types::object::page::PageResponse> for AnkiEntity {
    type Error = http_api_core::error::Error;
    ...
    fn try_from(
        ...
        let title_property =
            properties
                .get("title")
                .ok_or(http_api_core::error::Error::NotionPropertyNotFound(
                    "title".to_string(),
                ))?;
        ...
        let title = match title_property {
            ...
            _ => {
                return Err(http_api_core::error::Error::NotionPropertyNotFound(
                    "title".to_string(),
                ));
            }
        };
        ...
        // >>> description
        let description_property = properties.get("description").ok_or(
            http_api_core::error::Error::NotionPropertyNotFound("description".to_string()),
        )?;
        ...
        // >>> ease_factor
        let ease_factor_property = properties.get("easeFactor").ok_or(
            http_api_core::error::Error::NotionPropertyNotFound("easeFactor".to_string()),
        )?;
        ...
        let repetition_count_property = properties.get("repetitionCount").ok_or(
            http_api_core::error::Error::NotionPropertyNotFound("repetitionCount".to_string()),
        )?;

        let repetition_count = match repetition_count_property {
            notionrs_types::object::page::PageProperty::Number(repetition_count) => {
                repetition_count.number.ok_or(
                    http_api_core::error::Error::NotionPropertyNotFound(
        ...
        // >>> next_review_at
        let next_review_at_property = &properties.get("nextReviewAt").ok_or(
            http_api_core::error::Error::NotionPropertyNotFound("nextReviewAt".to_string()),
        )?;
        ...
        let tags = match tags_property {
            notionrs_types::object::page::PageProperty::MultiSelect(tags) => tags
                ...
                .collect::<Result<Vec<AnkiTagEntity>, http_api_core::error::Error>>(),
            _ => {
                return Err(http_api_core::error::Error::NotionPropertyNotFound(
                    "tags".to_string(),
                ));
        ...
        let review_required = &properties.get("isReviewRequired").ok_or(
            http_api_core::error::Error::NotionPropertyNotFound("isReviewRequired".to_string()),
        )?;
