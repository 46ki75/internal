...
pub struct BookmarkState {
    pub bookmark_use_case: Arc<crate::use_case::BookmarkUseCase>,
}

pub async fn init_bookmark_router()
-> Result<(axum::Router, utoipa::openapi::OpenApi), http_api_core::error::Error> {
    let repository = Arc::new(crate::repository::BookmarkRepositoryImpl {});
    let use_case = Arc::new(crate::use_case::BookmarkUseCase {
...
pub fn bookmark_router(state: Arc<BookmarkState>) -> (axum::Router, utoipa::openapi::OpenApi) {
    OpenApiRouter::new()
        .routes(routes!(crate::controller::bookmark_list))
        .routes(routes!(crate::controller::create_bookmark))
        .with_state(state)
        .split_for_parts()
}
