...
impl RoutineQuery {
    pub async fn list_routine(
        ...
        input: Option<RoutineQueryInput>,
        ...
        let mut request = client.query_database().database_id(database_id);

        if let Some(input) = input {
            if let Some(day_of_week) = input.day_of_week {
                request = request.filter(notionrs::filter::Filter::multi_select_contains(
                    "DayOfWeek",
                    day_of_week,
                ));
            }
